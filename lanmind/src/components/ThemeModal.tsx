...
export const ThemeModal: React.FC<ThemeModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="max-h-[calc(100vh-2rem)] w-full max-w-lg space-y-5 overflow-y-auto rounded-2xl border border-slate-800 bg-slate-900 p-6 shadow-2xl animate-in fade-in zoom-in-95 duration-150">
        {/* Modal Header */}
        ...
        <button
          ...
          className="flex w-full items-center justify-between gap-4 rounded-xl border border-slate-800 bg-slate-950/60 p-3.5 text-left transition-colors hover:border-slate-700 hover:bg-slate-800/50"
        >
              ...
              >
                <div>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    {isSelected && (
                      <span
                        className="w-5 h-5 rounded-full text-white flex items-center justify-center text-xs shadow-md"
                ...
                <div className="mt-3 pt-2 border-t border-slate-800/60 flex items-center justify-between text-[10px] text-slate-500 font-mono">
                  ...
                  <span className="rounded bg-slate-800/60 px-1.5 py-0.5 text-[9px] text-slate-400 border border-slate-700/50">
        ...
        <div className="flex items-center justify-between pt-3 border-t border-slate-800">
          <div className="flex items-center space-x-1.5 text-xs text-slate-400">
            ...
            <span>
              当前活跃: <strong className="text-white">{currentTheme.name}</strong>
